...
describe("admin console routes", () => {
  ...
  it("serves a login form when enabled without leaking the admin token", async () => {
    ...
    expect(response.body).toContain("Stellive Hub Admin");
    expect(response.body).toContain("Enter your admin console token to continue.");
    expect(response.body).toContain("Authorized administrators only.");
    ...
    expect(response.body).toContain('<svg class="password-icon" aria-hidden="true" focusable="false" viewBox="0 0 24 24">');
    expect(response.body).toContain("--login-action-bg: #c7d2fe");
    expect(response.body).toMatch(/<footer class="login-utility">[\s\S]*class="theme-control"[\s\S]*<nav class="language-control"[\s\S]*<\/footer>\s*<\/form>/);
    ...
    expect(response.body.match(/<input /g)).toHaveLength(1);
    expect(response.body).not.toContain('class="login-brand-mark"');
    expect(response.body).not.toContain('class="login-badge"');
    expect(response.body).not.toContain('class="private-badge"');
    expect(response.body).not.toContain('class="login-flow"');
    expect(response.body).not.toMatch(/>\s*lock\s*</i);
    expect(response.body).not.toContain("Internal token later");
    expect(response.body).not.toContain("Internal API bearer token");
