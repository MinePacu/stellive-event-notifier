...
async function main(): Promise<void> {
  ...
  if (!fcmClient.enabled) {
    console.error(JSON.stringify({ operation: "service_topic_reconciliation", status: "failed", reason: "fcm_disabled" }));
    process.exitCode = 1;
    return;
  }
  ...
  const summary = await reconcileServiceTopicSubscriptions({
    devices,
    subscriptions: service,
  });
  console.info(JSON.stringify({ operation: "service_topic_reconciliation", ...summary }));
...
try {
  ...
  console.error(JSON.stringify({ operation: "service_topic_reconciliation", status: "failed", reason: "unexpected_error" }));
  process.exitCode = 1;
} finally {
