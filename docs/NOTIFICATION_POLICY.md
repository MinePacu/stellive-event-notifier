...
Service-wide announcements are persisted as `service_announcement` platform events and delivered through the normal per-device notification job and worker path. The worker resolves the latest global and `serviceAnnouncementsEnabled` settings immediately before each provider send, so a queued announcement is skipped after either setting is turned off. No topic subscription or topic publisher is used. At publish/resend time, the event snapshots `targetPlatforms`, `minimumAppVersion`, `maximumAppVersion`, and the targeting-policy version; the worker applies that snapshot per device before preference resolution and push. Platform matching is exact, and minimum/maximum app-version bounds are inclusive. When a bound exists, a missing or malformed device app version is skipped; with no bounds, a missing app version is allowed. Events without targeting metadata (including legacy events) fail closed with an explicit skipped delivery reason rather than broadening delivery. Service announcements retain their target platform list, but ignore member/platform/event-type rules, quiet hours, keyword filters, rate limits, and load-reduction downgrades. The legacy cleanup job only unsubscribes active tokens from the allowlisted legacy topics, retries provider failures without logging tokens, and reports aggregate failures for a later retry.
...
The unique-token migration deterministically preserves the most recently updated active duplicate because the actual physical installation cannot be recovered from stored data alone. Deploy the transaction/CAS-compatible code first, then pause notification workers and run `prisma migrate deploy`. Run the one-time `serviceTopicCleanupWorker` from `backend/stellive-hub-api` to remove legacy topic subscriptions in bounded batches; it does not print tokens and exits non-zero when provider failures remain unresolved. New announcement sends are independent of cleanup success.
