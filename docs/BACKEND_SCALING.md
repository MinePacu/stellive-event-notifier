...
`docker-compose.scale.yml`은 기존 단일 `api`를 `single-api` profile로 제외하고 정적 `api-1`~`api-4`와 Nginx를 추가합니다. Nginx 1.27은 Docker DNS로 각 정적 service를 다시 resolve하므로 2개만 실행한 경우에도 없는 `api-3`, `api-4`를 활성 upstream으로 취급하지 않습니다. 외부에는 Nginx의 `4000`만 공개됩니다.
