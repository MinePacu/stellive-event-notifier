...
This file is archived historical context only.

Agents must not read this file during normal feature work, merge-only work, routine debugging, or routine PR review.

Use only when the user explicitly requests historical context, when investigating a regression that requires old implementation history, or when a current handoff document links to this exact file.

The content below preserves the previous `docs/AI_HANDOFF.md` verbatim as archived June-era project history.
...
Latest verification on this branch: backend `rtk npm run build` and `rtk npm test` passed with 28 files and 260 tests; iOS `test_sim` passed on `iPhone 17` with 38 tests; Android `rtk ./gradlew testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarWidgetTextFormatterTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` and full `rtk ./gradlew testDebugUnitTest` passed from `android/StelliveHubAndroid`.
...
Verification:
- Android RED: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` failed before implementation because the new policy helpers did not exist.
- Android focused GREEN: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.HubEventsCalendarViewModelTest` passed.
...
Known verification gap:
- Android focused test `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` could not run in sandbox because Gradle wrapper needed `/Users/nohyunsoo/.gradle/.../gradle-9.0.0-bin.zip.lck` write access. Escalated retry was rejected by policy, so Android post-implementation tests remain unverified in this session.
...
- iOS build/install/run GREEN: XcodeBuildMCP `build_run_sim` succeeded on booted `iPhone 17 Pro` simulator `89B0B46A-8515-47E7-A122-681498F16C66`; app path `/Users/nohyunsoo/Library/Developer/XcodeBuildMCP/workspaces/StelLiveNoti-05b02c56ff42/DerivedData/StelliveHubiOS-6035de198454/Build/Products/Debug-iphonesimulator/StelliveHubiOS.app`, bundle id `dev.minepacu.stelliveeventnotifier`, process id `97204`.
...
- Android build/install/run GREEN after explicit user approval: `rtk ./gradlew :app:assembleDebug` passed, `rtk adb devices` found `100.76.105.15:44105`, `rtk adb -s 100.76.105.15:44105 install -r android/StelliveHubAndroid/app/build/outputs/apk/debug/app-debug.apk` returned `Success`, and `rtk adb -s 100.76.105.15:44105 shell am start -n dev.minepacu.stelliveeventnotifier/.MainActivity` launched the app.
- Android runtime verification: package `dev.minepacu.stelliveeventnotifier` is installed, resolved activity is `dev.minepacu.stelliveeventnotifier/.MainActivity`, process id `13322` was observed after launch, foreground window showed `dev.minepacu.stelliveeventnotifier/dev.minepacu.stelliveeventnotifier.MainActivity`, and crash buffer was empty after clearing old unrelated crash entries.
...
- Android focused GREEN: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` passed after adding a range-title formatter test.
- Android focused GREEN: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` passed again after feed header wiring changes. A parallel `assembleDebug` attempt hit Kotlin incremental cache contention; rerunning `assembleDebug` alone passed.
- Android simplified build/install/run GREEN: `rtk ./gradlew :app:assembleDebug` passed, `rtk adb -s 100.76.105.15:44105 install -r android/StelliveHubAndroid/app/build/outputs/apk/debug/app-debug.apk` returned `Success`, `rtk adb -s 100.76.105.15:44105 shell am start -n dev.minepacu.stelliveeventnotifier/.MainActivity` started the activity, and `pidof` returned `18760`.
- iOS build/run GREEN: XcodeBuildMCP `build_run_sim` succeeded on `iPhone 17 Pro` simulator `89B0B46A-8515-47E7-A122-681498F16C66`, bundle id `dev.minepacu.stelliveeventnotifier`, process id `4965`.

## Admin Hub Event Image Form Preserve Fields

Plan: `docs/superpowers/plans/2026-06-21-admin-hub-event-image-form-preserve-fields-code-design.md`. Root cause: admin console draft save rebuilt `input.image` as `null` whenever `imagePolicyState === "none"`, so filled `Image URL`, `Image source label`, and `Image source URL` could be silently discarded. Fix: `collectHubEventInput()` now rejects `none` plus filled image metadata before sending the request, while still allowing `image: null` when all image fields are empty. Backend validation also rejects `image.policyState = "none"` with review metadata so API callers cannot bypass the guard. `verify_required` and `blocked` may preserve URL/source metadata for review but remain non-displayable to mobile clients. Files changed: `backend/stellive-hub-api/src/admin/adminConsoleHtml.ts`, `backend/stellive-hub-api/src/hub-events/hubEventPolicy.ts`, `backend/stellive-hub-api/test/hubEventImagePolicy.test.ts`, `backend/stellive-hub-api/test/adminHubEventRoutes.test.ts`. Note: the plan named `adminInternalRoutes.test.ts`, but Hub Event CRUD draft-save behavior is covered by `adminHubEventRoutes.test.ts`, so the round-trip test was added there. Verification: RED `rtk npm test -- hubEventImagePolicy` failed before implementation on the new `none` metadata rejection tests; GREEN `rtk npm test -- hubEventImagePolicy`, `rtk npm test -- adminHubEventRoutes`, `rtk npm run build`, `rtk npm test -- hubEventImagePolicy adminHubEventRoutes`, and final `rtk npm test` passed. Policy grep on backend src/tests plus the plan matched only existing catalog guards, rawPayload internals/tests, and explicit forbidden image-field validation references; no secrets, unauthorized assets, Former catalog entries, login-cookie scraping, or new prohibited media fields were introduced.

## Hub Calendar Event Card Dedupe

Plan: `docs/superpowers/plans/2026-06-21-calendar-event-card-dedupe-code-design.md`. Root cause: backend `/v1/hub-events/calendar` intentionally expands multi-day HubEvents into date-level entries for markers and duration bars, while Android/iOS range/month card lists flattened those entries without grouping by canonical `eventId`. Fix: Android `CalendarUiPolicy.entriesForRange()` dedupes sorted range entries by `eventId`; iOS `HubEventsCalendarViewModel.selectedMonthEntries()` and range `entries(in:)` dedupe by `eventId`; day-level entries, markers, and duration bar layout stay date-expanded. Files changed: `android/StelliveHubAndroid/app/src/main/java/dev/minepacu/stelliveeventnotifier/feature/calendar/CalendarUiPolicy.kt`, `android/StelliveHubAndroid/app/src/test/java/dev/minepacu/stelliveeventnotifier/CalendarUiPolicyTest.kt`, `android/StelliveHubAndroid/app/src/test/java/dev/minepacu/stelliveeventnotifier/HubEventsCalendarViewModelTest.kt`, `ios/StelliveHubiOS/StelliveHubiOS/Views/HubEventsCalendarViewModel.swift`, `ios/StelliveHubiOS/StelliveHubiOSTests/HubEventsCalendarViewModelTests.swift`. RED Android `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` failed with duplicate `[goods-range, goods-range]`; GREEN Android focused `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.HubEventsCalendarViewModelTest` passed. RED iOS focused XCTest failed on `testVisibleEntriesDeduplicatesMultiDayCalendarEntriesByEventId`; GREEN iOS focused XCTest on simulator `89B0B46A-8515-47E7-A122-681498F16C66` exited `status=0`.

## Calendar Feed Canonical Event Dedupe

Plan: `docs/superpowers/plans/2026-06-21-calendar-feed-canonical-event-dedupe-code-design.md`. Root cause: app Goods/Events feeds still used date-expanded `/v1/hub-events/calendar` rows in some paths. Backend canonical `/v1/hub-events` remains unique and image-bearing, while calendar rows are intentionally expanded for markers/bars. iOS also fell back to `HubCalendarRow` when `cachedHubEvent` missed, causing image-less feed cards. Fix: Android feed projection now dedupes by `eventId` and renders canonical `HubEvent` cards only; Android/iOS no longer send built-in UI filters (`all`, `goods`, `ticketing`, `offline`, `closing`) as backend `generationId`; iOS `HubEventsView` now projects selected-month feed rows via `HubEventsFeedPolicy`, groups renderable rows into sections, and only falls back to `HubCalendarRow` for non-hub calendar entries. Files changed: `android/StelliveHubAndroid/app/src/main/java/dev/minepacu/stelliveeventnotifier/MainActivity.kt`, `android/StelliveHubAndroid/app/src/main/java/dev/minepacu/stelliveeventnotifier/feature/calendar/CalendarUiPolicy.kt`, `android/StelliveHubAndroid/app/src/main/java/dev/minepacu/stelliveeventnotifier/feature/home/ServerHubRepository.kt`, `android/StelliveHubAndroid/app/src/test/java/dev/minepacu/stelliveeventnotifier/CalendarUiPolicyTest.kt`, `android/StelliveHubAndroid/app/src/test/java/dev/minepacu/stelliveeventnotifier/ServerHubRepositoryTest.kt`, `ios/StelliveHubiOS/StelliveHubiOS/Views/HubEventsView.swift`, `ios/StelliveHubiOS/StelliveHubiOS/Services/ServerHubStore.swift`, `ios/StelliveHubiOS/StelliveHubiOSTests/HubEventsCalendarViewModelTests.swift`, `ios/StelliveHubiOS/StelliveHubiOSTests/HubAPIClientTests.swift`. Verification: Android focused GREEN `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.ServerHubRepositoryTest` passed after escalated Gradle cache access. iOS focused GREEN via XcodeBuildMCP `test_sim` on booted iPhone 17 Pro `89B0B46A-8515-47E7-A122-681498F16C66`: 40 passed, 0 failed. Server contract GREEN: `/v1/hub-events?limit=100` returned 3 unique ids and all had `image: true`; `/v1/hub-events/calendar` for June 2026 remained expanded with duplicate date rows for `qg9oqjt5rh3ijhaiqnwvlj8q` count 13 and `c13yk3lroxky6ezp00ooduh0` count 6. Install verification GREEN: Android `assembleDebug` passed, adb device `100.76.105.15:35039` installed successfully, launch succeeded, and `pidof dev.minepacu.stelliveeventnotifier` returned `19779`; iOS XcodeBuildMCP `build_run_sim` succeeded on simulator `89B0B46A-8515-47E7-A122-681498F16C66` with bundle `dev.minepacu.stelliveeventnotifier`, process id `42278`. Policy grep matched only existing policy/model/test references and the plan text; `rtk git diff --check` passed.

## Android Special-Day Feed Cards

Plan: `docs/superpowers/plans/2026-06-21-android-special-day-feed-cards-code-plan.md`. Root cause: Android Goods/Events feed resolved every calendar row through canonical `/v1/hub-events` IDs, so `member_birthday` and `generation_anniversary` rows such as `birthday:sakihane-huya` were dropped. Fix: `CalendarUiPolicy.feedRenderRowsForMonth()` keeps special-day rows without canonical `HubEvent` and `MainActivity.renderServerGoodsEvents()` renders them with `localCalendarEntryRow()`. Verification: RED `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` failed before implementation because `feedRenderRowsForMonth` did not exist; GREEN focused `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.CalendarUiPolicyTest` passed after implementation; `rtk git diff --check` passed. `assembleDebug`, adb install, backend tests, and iOS tests were not run because this plan marked them as conditional or out of scope.
...
Verification:
- RED: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.SongUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.ServerHubRepositoryTest` failed after escalation because song DTOs/repository APIs/policy helpers did not exist.
- GREEN: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.SongUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.ServerHubRepositoryTest --tests dev.minepacu.stelliveeventnotifier.MainUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.HubApiClientTest` passed after implementation.
...
- Backend build GREEN: `rtk npm run build` passed from `backend/stellive-hub-api`.
- Android focused GREEN: `rtk ./gradlew :app:testDebugUnitTest --tests dev.minepacu.stelliveeventnotifier.SongUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.ServerHubRepositoryTest --tests dev.minepacu.stelliveeventnotifier.MainUiPolicyTest --tests dev.minepacu.stelliveeventnotifier.HubApiClientTest` passed after escalated Gradle cache access.
- Diff hygiene GREEN: `rtk git diff --check` passed.
