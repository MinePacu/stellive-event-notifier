...
#### Added
- 굿즈·행사에 `album` 보조 태그를 추가하고, 관리자 콘솔에서 음반 태그를 작성·수정·필터링할 수 있도록 했습니다.
- 공개 허브 이벤트 API와 Prisma 저장소에 음반 태그 조회 및 저장을 지원하고, 달력·위젯 응답에도 태그를 투영했습니다.
- Android와 iOS 굿즈·행사 목록 및 달력에 음반 필터와 음반 보조 칩을 추가했습니다.
- iOS 허브 이벤트 상세에 캘린더 카드와 정책을 추가했습니다.
- Android 허브 이벤트 상세에 캘린더 카드와 정책을 추가했습니다.
- Android 노래 화면 two-pane에서 pull-to-refresh 정책을 추가했습니다.
- Android 노래 화면 강제 새로고침 및 pull-to-refresh 수정 사항을 추가했습니다.
- 공유 도메인 타입과 OpenAPI 스펙에 음악 세대(생성) 메타데이터를 추가했습니다.
- Android 음악 API에 세대 메타데이터와 새로고침 지원을 추가했습니다.
- iOS 음악 API에 세대 메타데이터와 강제 새로고침을 추가했습니다.
- 백엔드 음악 API에 세대 메타데이터와 캐시 무효화를 추가했습니다.

#### Changed
- 태그 입력의 공백·중복을 정규화하고, 미지원 태그와 잘못된 관리자 입력을 검증하도록 했습니다.
- 태그가 누락된 구버전 서버 응답은 빈 배열로 처리하고, 알 수 없는 미래 태그는 행사 전체를 버리지 않고 무시하도록 모바일 변환을 보강했습니다.
- 기존 카테고리·상태·알림 동작을 유지하면서 `online_goods` 및 `ticketing` 행사에서 음반 태그 필터를 독립적으로 적용했습니다.
- iOS 노래 뷰에서 리스트 구분선을 복원하고 SongRow 배경을 단순화했습니다.
- iOS 노래 뷰에서 리스트 구분선을 숨기고 행 배경을 투명하게 처리했습니다.
- 백엔드 음악 동기화에 상세 카운트와 캐시 무효화를 강화했습니다.
- 문서를 구조화된 알림 정책 표와 프로젝트 개요로 재작성했습니다.

#### Fixed
- 백엔드 음악 동기화 카운트와 캐시 무효화 테스트를 추가했습니다.

#### CI
- 문서-위키 동기화 워크플로를 추가했습니다.
- README에 위키 링크를 추가하고 위키 동기화 워크플로를 추가했습니다.

#### Documentation
- 공유 도메인 계약과 OpenAPI 스키마에 `HubEventTag` 및 `tags` 필드를 반영했습니다.

### 2026-07-25

#### Added
- Android와 iOS 예약·구매 도움말에 굿즈·행사 링크 열기부터 외부 예매·결제·예약, 앱 내역 추가까지의 3단계 사용 흐름과 빈 목록의 사용 방법 진입점을 추가했습니다.

#### Changed
- 확인 필요 임시 항목의 최대 2시간 유지, 외부 완료 여부 미검증, 링크 없이 추가, 로컬 저장·백업 제외를 명확히 안내하도록 도움말 구조와 의미별 정보·주의·보안 표현을 개선했습니다.
- 내역 상세 도움말에 링크 우선순위, 사용자 수정값 우선 표시, 공식 행사 정보 미자동 반영, 앱 내역 삭제와 외부 예약 취소의 차이를 추가하고 Android·iOS 접근성 진입점을 정리했습니다.

### 2026-07-21

#### Changed
- 멤버별 YouTube Playlist 편집본을 커버곡·오리지널곡 목록에 자동 분류하고, 제목의 Unicode 호환 문자를 정규화해 분류 안정성을 높였습니다.

#### Fixed
- 커버곡·오리지널곡 목록이 실제 YouTube 게시일 최신순(`publishedAt DESC`, null 마지막, 동일 게시일은 ID 오름차순)으로 정렬되고, 커서 페이지네이션도 같은 기준을 유지하도록 수정했습니다.

### 2026-07-20

#### Changed
- Android/iOS 굿즈/행사 피드는 부모 행사를 한 번만 표시하고, 캘린더와 상세 화면에서는 세부 일정별 제목·기간·상태·선택 정보를 유지하도록 정리했습니다.
- Android/iOS 행사 상세의 세부 일정 태그와 상태 표현을 의미별 색상으로 다듬고, 일정 카드 펼침·접힘 애니메이션과 화살표 상태 표시를 개선했습니다.
- 행사 변경 경로와 관련 모바일 회귀 테스트 범위를 보강했습니다.

#### Fixed
- Android 굿즈/행사 상세 외부 링크가 패키지 공개 상태에 따른 처리 앱 사전 조회 실패로 열리지 않던 문제를 수정했습니다.
